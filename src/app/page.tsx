    ...
    <main className="min-h-screen flex flex-col items-center justify-center auth-gradient p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center w-full max-w-6xl">
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left z-10">
          <h1 className="text-4xl md:text-5xl font-bold text-foreground font-headline mb-2">
            Welcome to ProHub
          </h1>
          <p className="text-lg md:text-xl text-muted-foreground mb-8 lg:mb-10 max-w-md">
          ...
          <div className="bg-card/70 dark:bg-card/80 backdrop-blur-md p-8 rounded-2xl shadow-xl w-full max-w-md border border-border/30 dark:border-border/50">
            ...
            <ul className="space-y-5">
              {features.map((feature) => (
                <li key={feature.title} className="flex items-start gap-4 p-4 bg-background/50 dark:bg-muted/20 rounded-xl hover:bg-background/70 dark:hover:bg-muted/30 transition-colors duration-200 shadow-sm">
      ...
      <div className="absolute top-0 left-0 w-72 h-72 bg-primary/5 rounded-full opacity-30 -translate-x-1/2 -translate-y-1/2 filter blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-0 right-0 w-72 h-72 bg-accent/5 rounded-full opacity-30 translate-x-1/2 translate-y-1/2 filter blur-3xl pointer-events-none"></div>
    </main>
  );
}
